...
# from src.database.users import User # Закомментировано, так как класс переопределен ниже
from src.database.store import Storage
from src.database.requests import Requests


# --- МОДЕЛИ ДАННЫХ (Включая обновленный User с кошельком) ---
...
class User(db.Model):
    ...
    def __init__(self, login, password, role="student", wallet=None, email=None):
        ...
        # Берем ключ из окружения или генерируем временный (чтобы не падало)
        key = os.getenv("KEY", Fernet.generate_key().decode())
        # Приводим к байтам, если это строка
    ...
    def get_wallet(self):
        key = os.getenv("KEY", Fernet.generate_key().decode()) # Лучше вынести ключ в конфиг, но пока так
        try:
            k = key.encode() if isinstance(key, str) else key
            cipher_suite = Fernet(k)
...
# В продакшене этот ключ должен быть скрыт в .env файле!
